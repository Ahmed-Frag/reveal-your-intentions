...
export default function SmartReplyCard({ show = false, result }) {
    ...
    return btoa(
      encodeURIComponent(
        JSON.stringify({
          ...
          smart: result?.smart_reply?.written || "",
        })
      )
    ).slice(0, 80);
  }, [result]);
  ...
  const variants = {
    smart: {
      ...
      written:
        generatedReplies.smart?.written ||
        result?.smart_reply?.written ||
        "لا يوجد رد كتابي مقترح حاليًا.",
      face:
        generatedReplies.smart?.face ||
        result?.smart_reply?.face_to_face ||
        "لا يوجد توجيه وجهًا لوجه حاليًا.",
    },
    strong: {
      label: "أقوى",
      icon: Flame,
      tone: "يثبت حدودك",
      written:
        generatedReplies.strong?.written ||
        "اضغط لتجهيز الرد الأقوى.",
      face:
        generatedReplies.strong?.face ||
        "اضغط لتجهيز التوجيه المباشر.",
    },
    calm: {
      label: "أهدى",
      icon: Leaf,
      tone: "يهدّي الموقف",
      written:
        generatedReplies.calm?.written ||
        "اضغط لتجهيز الرد الأهدى.",
      face:
        generatedReplies.calm?.face ||
        "اضغط لتجهيز التوجيه المباشر.",
    },
    cold: {
      label: "أبرد",
      ...
      written:
        generatedReplies.cold?.written ||
        "اضغط لتجهيز الرد الأبرد.",
      face:
        generatedReplies.cold?.face ||
        "اضغط لتجهيز التوجيه المباشر.",
    },
  };
  ...
  const handleSelect = async (key) => {
    ...
    const storageKey = `reply_variant_${resultKey}_${key}`;
    ...
    try {
      const response = await fetch("/api/reply-variant", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !data?.success) {
        throw new Error(data?.message || "فشل تجهيز الرد");
      }

      const reply = {
        written: data.data.written,
        face: data.data.face_to_face,
      };

      sessionStorage.setItem(storageKey, JSON.stringify(reply));
      ...
      console.error("Reply Variant Error:", error);
    } finally {
      setLoadingVariant(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="rounded-[28px] border border-purple-400/30 bg-gradient-to-br from-purple-500/30 via-white/25 to-blue-500/20 p-[1px] shadow-[0_14px_42px_rgba(124,58,237,0.14)] dark:border-purple-500/25 dark:via-white/5">
        <div className="rounded-[27px] border border-[var(--surface-border)] bg-[var(--surface-main)] p-4 backdrop-blur-md dark:border-white/10 dark:bg-[#070b1d]/80 sm:p-5">
          <div className="mb-5 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-purple-500 to-blue-500 text-white shadow-[0_0_18px_rgba(168,85,247,0.32)]">
                <Sparkles size={21} />
              </div>
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${active
                        ? "border-purple-500/45 bg-purple-500/15 text-purple-700 shadow-[0_0_14px_rgba(168,85,247,0.12)] dark:text-purple-200"
...
function ReplyBox({ icon, title, text, copied, onCopy }) {
  return (
    <div className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-4 dark:border-white/10 dark:bg-white/[0.04]">
      ...
      <p className="text-sm font-bold leading-7 text-slate-900 dark:text-white sm:text-base whitespace-pre-line">
        {text}
      </p>
    </div>
  );
