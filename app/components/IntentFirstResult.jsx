...
export default function IntentFirstResult({ show = false, result }) {
  if (!show) return null;
      ...
      >
        ...
        >
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <MistakesBlock mistakes={result?.mistakes || []} />

            <StrategyBlock smartMoves={result?.smart_moves || []} />
          </div>
        </div>
      </div>
    </div>
...
function MistakesBlock({ mistakes }) {
  return (
    <div
      ...
      "
    >
      <div className="absolute -left-10 -top-10 h-28 w-28 rounded-full bg-rose-500/20 blur-2xl" />
...
function StrategyBlock({ smartMoves = [] }) {
  return (
    <div
      className="
        relative overflow-hidden rounded-2xl border p-4
        border-emerald-400/30 bg-emerald-500/10
        shadow-[0_0_18px_rgba(16,185,129,0.09)]
      ...
      <div className="relative z-10">
        ...
        <div className="space-y-2.5">
          {smartMoves.map((item) => (
            <div
              key={item}
              className="flex items-start gap-2 rounded-xl border border-emerald-400/20 bg-white/35 p-2.5 dark:bg-white/[0.04]"
            >
              <CheckCircle2
                size={17}
